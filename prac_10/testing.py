...
def run_tests():
    ...
    # assert test to check Car's odometer
    car = Car("Test Car")
    assert car._odometer == 0, "Car does not set odometer correctly"

    # assert tests to check Car's fuel handling
    car_with_default_fuel = Car("Default Fuel Car")
    assert car_with_default_fuel.fuel == 0, "Default fuel is not 0"

    car_with_fuel = Car("Fuel Car", fuel=10)
